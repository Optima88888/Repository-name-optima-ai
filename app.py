...
APP_TITLE = "Mkt Automation Pro V8 Page Center Pro"
...
function openModule(moduleId){
  const moduleAlias = {
    "page_post":"page_center",
    "page_comment_uid":"page_center",
    "page_auto_post":"page_center",
  ...
  const trialAllowed = ["dashboard", "facebook_center", "page_center", "post", "fanpage_manager", "group_suite", "group_marketing", "comment_manager", "analytics", "automation_center", "premium"];
  const premiumLocked = {
...
function mktSafeScrollTo(moduleId){
  ...
  document.querySelectorAll(".v2-nav-link,.nav a").forEach(function(a){ a.classList.remove("active-nav"); });
  document.querySelectorAll("a[href='#"+moduleId+"']").forEach(function(a){ a.classList.add("active-nav"); });
  return true;
}
document.addEventListener("click", function(e){
  ...
  if(target){ e.preventDefault(); openModule(id); }
});
...
.active-nav{background:linear-gradient(135deg,#2563EB,#7C3AED)!important;color:#fff!important;border-color:#A78BFA!important}
</style>
...
<aside class="sidebar">
  ...
  <div class="subtitle">V8 Page Center • Group Center • Comment Manager</div>
...
<div class="nav">
  ...
  <a class="v2-nav-link" href="#page_center" onclick="return openModule('page_center')"><span class="v2-nav-ico">📌</span><span class="v2-nav-text">Page Center Tổng</span><span class="v2-nav-tag">V8</span></a>
  <a class="v2-nav-link" href="#post" onclick="return openModule('post')"><span class="v2-nav-ico">📝</span><span class="v2-nav-text">Đăng bài Facebook</span></a>
...
<div class="app-quick-grid">
  ...
  <div class="app-quick-card" onclick="openModule('page_center')">
    <div class="app-ico">📌</div>
    <b>Page Center Tổng</b>
    <span>Chọn nhiều Page, chia nội dung từng dòng, đăng ngay hoặc hẹn giờ 30p/1h/2h/3h.</span>
  </div>
...
<section class="panel module-section" id="page_center">
  <div class="section-open-note">Bạn đang mở: Page Center Tổng V8.</div>
  <h2>Page Center Tổng - đăng bài & bình luận UID trong một khung</h2>
  <p class="small">Gom chung: chọn nhiều Page, chia Page hoạt động, nhập nhiều nội dung mỗi dòng, đăng ngay hoặc hẹn giờ 30 phút / 1 giờ / 2 giờ / 3 giờ. Nội dung được chia theo từng dòng và không đăng trùng nội dung trong cùng một lệnh.</p>
  <div class="gf-warning">Bình luận UID người dùng chỉ đưa vào hàng chờ duyệt và ghi log. Chỉ xử lý với người dùng/bài viết/nhóm mà Page có quyền tương tác hợp lệ; không dùng để spam tự động.</div>

  <div class="gf-grid-3">
    <div class="gf-stat"><span>Page đã cấu hình</span><b>{{ pages|length }}</b></div>
  ...
  <div class="gf-box">
    ...
    <form method="post" action="/page_center_bulk_post" enctype="multipart/form-data">
      <div class="gf-grid">
        <div>
          <label>Chọn Page hoạt động</label>
          <div class="gf-box" style="max-height:230px;overflow:auto;padding:10px">
            {% for p in pages %}
            <label style="display:block;margin:7px 0"><input type="checkbox" name="page_indexes" value="{{ loop.index0 }}"> {{ p.name }} - {{ p.id }}</label>
            {% endfor %}
          </div>
          <small class="small">Có thể chọn nhiều Page. Hệ thống sẽ chia từng dòng nội dung cho Page theo vòng quay, không lặp lại nội dung.</small>
        </div>
        <div>
          ...
          <select name="mode">
            ...
            <option value="queue">Chỉ lưu hàng chờ duyệt</option>
          </select>
          <label>Hẹn giờ sau</label>
          <select name="schedule_after">
            <option value="30">30 phút</option>
            <option value="60">1 giờ</option>
            <option value="120">2 giờ</option>
            <option value="180">3 giờ</option>
          </select>
          <label>Khoảng cách giữa mỗi bài</label>
          <select name="gap_minutes">
            <option value="30">30 phút / bài</option>
            <option value="60">1 giờ / bài</option>
            <option value="120">2 giờ / bài</option>
            <option value="180">3 giờ / bài</option>
          </select>
          <input name="campaign" placeholder="Tên chiến dịch / ghi chú">
          <input type="file" name="images" multiple>
        </div>
      </div>
      <textarea name="contents" rows="8" placeholder="Mỗi dòng là 1 nội dung riêng. Ví dụ:\nNội dung bài 1...\nNội dung bài 2...\nNội dung bài 3..."></textarea>
      <button name="submit_action" value="now">Đăng ngay</button>
      <button name="submit_action" value="schedule">Hẹn giờ</button>
      <button class="secondary" name="submit_action" value="queue">Lưu hàng chờ</button>
    </form>
  </div>

  <div class="gf-box">
    <h3>2. Bình luận UID người dùng / UID bài viết - giãn cách 45 đến 90 giây</h3>
    <form method="post" action="/page_center_comment_queue">
      <div class="gf-grid">
        <div>
          <label>Chọn nhiều Page bình luận</label>
          <div class="gf-box" style="max-height:200px;overflow:auto;padding:10px">
            {% for p in pages %}
            <label style="display:block;margin:7px 0"><input type="checkbox" name="page_indexes" value="{{ loop.index0 }}"> {{ p.name }} - {{ p.id }}</label>
            {% endfor %}
          </div>
        ...
        <div>
          ...
          <select name="target_type"><option value="post">UID bài viết</option><option value="user">UID người dùng đã tương tác</option><option value="group">UID nhóm</option></select>
          <div class="gf-grid-3">
            <input name="min_delay" type="number" min="45" value="45" placeholder="Tối thiểu 45 giây">
            <input name="max_delay" type="number" min="45" value="90" placeholder="Tối đa 90 giây">
            <select name="approval"><option value="manual">Admin duyệt trước</option><option value="safe_queue">Chỉ lưu hàng chờ</option></select>
          </div>
        </div>
      </div>
      <textarea name="comments" rows="5" placeholder="Mỗi dòng là 1 bình luận khác nhau. Tool sẽ chia theo vòng quay, không lặp liên tục một nội dung."></textarea>
      <textarea name="raw_targets" rows="6" placeholder="Mỗi dòng là 1 UID. Với bài viết có thể nhập: post_uid, group_uid, user_uid"></textarea>
      <button>Tạo hàng chờ bình luận UID</button>
      <a class="btnlink" href="/export_page_comment_queue">Xuất CSV hàng chờ</a>
    </form>
  </div>

  <div class="gf-box">
    <h3>3. Lịch sử đăng Page gần nhất</h3>
    <div style="max-height:280px;overflow:auto">
      <table class="gf-table"><tr><th>Page</th><th>Nội dung</th><th>Trạng thái</th><th>Hẹn giờ</th><th>Chiến dịch</th></tr>
      {% for h in history %}<tr><td>{{ h[1] }}</td><td>{{ h[2][:120] }}</td><td>{{ h[3] }}</td><td>{{ h[5] }}</td><td>{{ h[7] }}</td></tr>{% endfor %}
      </table>
    </div>
  </div>
</section>
...
def split_lines_unique(raw_text):
    seen = set()
    result = []
...
def page_center_distribute_jobs(contents, pages):
    jobs = []
    if not contents or not pages:
        return jobs
...
@app.route("/page_center_bulk_post", methods=["POST"])
def page_center_bulk_post_route():
    pages = selected_pages(request.form.getlist("page_indexes"))
    contents = split_lines_unique(request.form.get("contents", ""))
    mode = request.form.get("submit_action") or request.form.get("mode", "now")
    campaign = request.form.get("campaign", "Page Center Tổng").strip()
    schedule_after = int(request.form.get("schedule_after", "30") or 30)
    gap_minutes = int(request.form.get("gap_minutes", "30") or 30)
    media_paths = save_uploads(request.files.getlist("images"))
    if not pages:
        return render(message="Chưa chọn Page hoạt động.", ok=False)
    if not contents:
        return render(message="Chưa nhập nội dung. Mỗi nội dung đặt trên 1 dòng.", ok=False)
    jobs = page_center_distribute_jobs(contents, pages)
    base_time = datetime.datetime.now() + datetime.timedelta(minutes=schedule_after)
    messages = []
    used_media = set()
    if mode == "queue":
        for idx, (page, content) in enumerate(jobs):
            schedule_time = (base_time + datetime.timedelta(minutes=idx * gap_minutes)).strftime("%Y-%m-%d %H:%M")
            image_path = choose_best_media_for_content(content, media_paths, used_media)
            save_post(page.get("name",""), page.get("id",""), content, "Chờ duyệt", "", schedule_time, image_path, campaign, score_content(content))
            messages.append(f"Đã lưu hàng chờ: {page.get('name')} • {schedule_time}")
        return render(message="\n".join(messages), ok=True)
    if mode == "schedule":
        for idx, (page, content) in enumerate(jobs):
            schedule_time = (base_time + datetime.timedelta(minutes=idx * gap_minutes)).strftime("%Y-%m-%d %H:%M")
            image_path = choose_best_media_for_content(content, media_paths, used_media)
            save_post(page.get("name",""), page.get("id",""), content, "scheduled", "", schedule_time, image_path, campaign, score_content(content))
            messages.append(f"Đã hẹn giờ {page.get('name')}: {schedule_time}")
        return render(message="\n".join(messages), ok=True)
    for idx, (page, content) in enumerate(jobs):
        token_status = check_single_page_token(page)
        if token_status.get("status") != "SỐNG":
            save_post(page.get("name",""), page.get("id",""), content, "error", token_status.get("detail","Token lỗi"), "", "", campaign, score_content(content))
            messages.append(f"Bỏ qua {page.get('name')}: token chưa hợp lệ")
            continue
        image_path = choose_best_media_for_content(content, media_paths, used_media)
        result = post_to_facebook(page, content, image_path)
        if "id" in result or "post_id" in result:
            post_id = result.get("post_id") or result.get("id")
            save_post(page.get("name",""), page.get("id",""), content, "posted", post_id, "", image_path, campaign, score_content(content))
            messages.append(f"Đăng thành công {page.get('name')}: {post_id}")
        else:
            save_post(page.get("name",""), page.get("id",""), content, "error", str(result), "", image_path, campaign, score_content(content))
            messages.append(f"Lỗi {page.get('name')}: {result}")
    return render(message="\n".join(messages), ok=True)

@app.route("/page_center_comment_queue", methods=["POST"])
def page_center_comment_queue_route():
    page_indexes = request.form.getlist("page_indexes")
    comments = split_lines_unique(request.form.get("comments", ""))
    raw_targets = split_lines_unique(request.form.get("raw_targets", ""))
    target_type = request.form.get("target_type", "post")
    min_delay = normalize_delay_seconds(request.form.get("min_delay", "45"), 45)
    max_delay = normalize_delay_seconds(request.form.get("max_delay", "90"), 90)
    if max_delay < min_delay:
        max_delay = min_delay
    if not page_indexes:
        return render(message="Chưa chọn Page để tạo hàng chờ bình luận.", ok=False)
    if not comments:
        return render(message="Chưa nhập nội dung bình luận, mỗi bình luận đặt trên 1 dòng.", ok=False)
    if not raw_targets:
        return render(message="Chưa nhập UID đích.", ok=False)
    added = 0
    skipped = 0
    base = datetime.datetime.now() + datetime.timedelta(seconds=min_delay)
    for pidx, page_index in enumerate(page_indexes):
        for tidx, line in enumerate(raw_targets):
            parts = [x.strip() for x in line.replace('\t', ',').split(',')]
            uid1 = normalize_uid(parts[0] if parts else '')
            uid2 = normalize_uid(parts[1] if len(parts) > 1 else '')
            uid3 = normalize_uid(parts[2] if len(parts) > 2 else '')
            if not uid1:
                skipped += 1
                continue
            comment_text = comments[(pidx + tidx) % len(comments)]
            user_uid = post_uid = group_uid = ''
            if target_type == 'user':
                user_uid = uid1
                ...
                group_uid = uid2
                user_uid = uid3
            scheduled_at = (base + datetime.timedelta(seconds=(added * min_delay))).strftime('%Y-%m-%d %H:%M:%S')
            qid = add_page_comment_queue(page_index, target_type, user_uid, post_uid, group_uid, comment_text, min_delay, max_delay, scheduled_at)
            if qid:
                added += 1
            else:
                skipped += 1
    return render(message=f"Đã tạo {added} hàng chờ bình luận UID. Bỏ qua {skipped}. Giãn cách {min_delay}-{max_delay} giây. Cần admin duyệt trước khi xử lý.", ok=True)
