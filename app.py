...
<style id="mkt-mobile-quick-actions-final-css">
  ...
  @media(max-width:768px){
    /* Ẩn các nút cài app/CTV cũ để không đè lên nội dung */
    #mktTopDownloadBar,
    #mobileCtvQuickBtn,
    ...
    body:not(.mkt-app-installed){padding-top:0!important;}
    ...
    #mktMobileQuickActions{
      ...
      font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;
      animation:mktQuickFloat 2.6s ease-in-out infinite!important;
    }
    #mktMobileQuickActions button{
      border:0!important;
      outline:0!important;
      ...
      color:#fff!important;
      box-shadow:0 12px 26px rgba(2,6,23,.24),inset 0 1px 0 rgba(255,255,255,.22)!important;
      backdrop-filter:blur(12px)!important;
      transform:translateZ(0)!important;
    }
    #mktMobileInstallQuick{
      padding:8px 13px!important;
      background:linear-gradient(135deg,#0284c7,#2563eb 48%,#7c3aed)!important;
      min-width:142px!important;
    }
    #mktMobileInstallQuick .mkt-q-dot{
      width:10px!important;height:10px!important;border-radius:50%!important;
      background:#34d987!important;box-shadow:0 0 0 4px rgba(52,217,135,.15),0 0 12px rgba(52,217,135,.78)!important;
      flex:0 0 10px!important;
    }
    #mktMobileInstallQuick .mkt-q-text{display:flex!important;flex-direction:column!important;align-items:flex-start!important;line-height:1.02!important;}
    #mktMobileInstallQuick .mkt-q-title{font-size:13px!important;color:#fff!important;white-space:nowrap!important;}
    #mktMobileInstallQuick .mkt-q-sub{font-size:10.5px!important;color:#dbeafe!important;white-space:nowrap!important;margin-top:2px!important;}
    #mktMobileCtvQuick{
    ...
    #mktMobileQuickActions button:active{transform:scale(.96)!important;}
    @keyframes mktQuickFloat{
      0%,100%{transform:translateY(0)}
      50%{transform:translateY(-3px)}
    }
    @media(max-width:390px){
      #mktMobileQuickActions{left:8px!important;gap:6px!important;}
      #mktMobileInstallQuick{min-width:128px!important;padding-left:11px!important;padding-right:11px!important;}
      #mktMobileInstallQuick .mkt-q-title{font-size:12.2px!important;}
      #mktMobileInstallQuick .mkt-q-sub{font-size:9.8px!important;}
      #mktMobileCtvQuick{min-width:52px!important;font-size:12px!important;padding-left:10px!important;padding-right:10px!important;}
    }
  }
</style>
<script id="mkt-mobile-quick-actions-final-js">
(function(){
  'use strict';
  var deferredPrompt = window.__mktDeferredPrompt || null;
  function qs(s,r){return (r||document).querySelector(s)}
  ...
  function ensureSheet(){
    var old=qs('#mktQuickInstallSheet'); if(old) return old;
    ...
    qs('#mktQuickInstallNow',wrap).onclick=runInstall;
    return wrap;
  }
  function guide(){
    var w=ensureSheet();
    ...
    if(isIOS()){
      ...
      var n2=qs('#mktQuickInstallNow',w); if(n2) n2.textContent='Cài ngay';
    }
  ...
  function openCTV(e){
    ...
    var target=qs('#affiliate_center')||qs('[data-module="affiliate_center"]')||qs('[id*="affiliate"]');
  ...
  window.addEventListener('appinstalled',function(){deferredPrompt=null;window.__mktDeferredPrompt=null;document.body.classList.add('mkt-app-installed')});
  document.addEventListener('click',function(e){
    var install=e.target.closest&&e.target.closest('#mktMobileInstallQuick,#mktTopDownloadBar,[data-install-app],#installAppBtn');
    if(install) return runInstall(e);
    var ctv=e.target.closest&&e.target.closest('#mktMobileCtvQuick');
    if(ctv) return openCTV(e);
  },true);
...
<!-- GPT MKT HARD FIX V4 - MAIN APP: Device ID + Mobile Download + Independent Support Chat -->
<style id="gpt-mkt-hard-fix-v4-css">
  #gptMktDeviceCardV4{margin:14px 18px 18px!important;padding:14px 14px!important;border:1px solid rgba(96,165,250,.35)!important;border-radius:18px!important;background:linear-gradient(135deg,rgba(37,99,235,.18),rgba(124,58,237,.16))!important;box-shadow:0 16px 36px rgba(0,0,0,.22)!important;color:#fff!important;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;}
  #gptMktDeviceCardV4 .t{font-size:11px!important;text-transform:uppercase!important;letter-spacing:.13em!important;color:#bfdbfe!important;font-weight:1000!important;margin-bottom:7px!important;}
  #gptMktDeviceCardV4 .id{font-size:14px!important;line-height:1.2!important;font-weight:1000!important;color:#fff!important;word-break:break-all!important;}
  #gptMktDeviceCardV4 .st{display:inline-flex!important;align-items:center!important;gap:6px!important;margin-top:9px!important;font-size:12px!important;color:#bbf7d0!important;font-weight:900!important;}
  #gptMktDeviceCardV4 .st:before{content:"";width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 14px #22c55e;}
  #gptMktInstallDockV4{position:fixed!important;left:14px!important;bottom:calc(18px + env(safe-area-inset-bottom,0px))!important;z-index:2147483600!important;display:flex!important;align-items:center!important;gap:8px!important;pointer-events:auto!important;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;}
  #gptMktInstallBtnV4{border:0!important;border-radius:18px!important;padding:11px 13px!important;min-width:112px!important;background:linear-gradient(135deg,#2563eb,#7c3aed)!important;color:#fff!important;box-shadow:0 18px 45px rgba(37,99,235,.42)!important;display:flex!important;align-items:center!important;gap:9px!important;cursor:pointer!important;pointer-events:auto!important;touch-action:manipulation!important;animation:gptMktFloatV4 2.8s ease-in-out infinite!important;}
  #gptMktInstallBtnV4 b{display:block!important;font-size:13px!important;line-height:1!important;font-weight:1000!important;}
  #gptMktInstallBtnV4 small{display:block!important;font-size:10px!important;line-height:1!important;margin-top:4px!important;color:#dbeafe!important;font-weight:900!important;}
  #gptMktInstallBtnV4 .ico{width:28px!important;height:28px!important;border-radius:12px!important;background:rgba(255,255,255,.18)!important;display:grid!important;place-items:center!important;font-size:16px!important;}
  #gptMktChatBtnV4{position:fixed!important;right:14px!important;bottom:calc(18px + env(safe-area-inset-bottom,0px))!important;z-index:2147483601!important;width:62px!important;height:62px!important;border-radius:24px!important;border:0!important;background:linear-gradient(135deg,#0f172a,#2563eb,#7c3aed)!important;color:#fff!important;box-shadow:0 20px 52px rgba(15,23,42,.55)!important;cursor:pointer!important;pointer-events:auto!important;touch-action:manipulation!important;display:grid!important;place-items:center!important;font-size:27px!important;animation:gptMktPulseV4 2.4s ease-in-out infinite!important;}
  #gptMktChatBtnV4:after{content:"";position:absolute;right:7px;top:7px;width:13px;height:13px;border-radius:50%;background:#22c55e;border:2px solid #fff;box-shadow:0 0 15px #22c55e;}
  #gptMktChatPanelV4{position:fixed!important;right:14px!important;bottom:calc(92px + env(safe-area-inset-bottom,0px))!important;width:min(390px,calc(100vw - 28px))!important;height:520px!important;max-height:calc(100vh - 125px)!important;z-index:2147483602!important;border-radius:26px!important;overflow:hidden!important;background:#fff!important;color:#0f172a!important;box-shadow:0 30px 90px rgba(15,23,42,.55)!important;border:1px solid rgba(148,163,184,.35)!important;display:none!important;pointer-events:auto!important;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif!important;}
  #gptMktChatPanelV4.open{display:flex!important;flex-direction:column!important;}
  .gptmkt-chat-head-v4{padding:15px 16px!important;background:linear-gradient(135deg,#0f172a,#2563eb,#7c3aed)!important;color:#fff!important;display:flex!important;align-items:center!important;justify-content:space-between!important;}
  .gptmkt-chat-head-v4 b{font-size:15px!important;font-weight:1000!important;display:block!important;}.gptmkt-chat-head-v4 small{display:block!important;color:#dbeafe!important;font-size:11px!important;font-weight:800!important;margin-top:3px!important;}
  .gptmkt-chat-close-v4{width:34px!important;height:34px!important;border-radius:12px!important;border:0!important;background:rgba(255,255,255,.16)!important;color:#fff!important;cursor:pointer!important;font-size:20px!important;}
  #gptMktChatBodyV4{flex:1!important;overflow:auto!important;padding:14px!important;background:linear-gradient(180deg,#f8fafc,#eef2ff)!important;}
  .gptmkt-msg-v4{max-width:86%!important;margin:0 0 10px!important;padding:10px 12px!important;border-radius:16px!important;font-size:13px!important;line-height:1.45!important;box-shadow:0 8px 22px rgba(15,23,42,.08)!important;white-space:pre-wrap!important;}
  .gptmkt-msg-v4.ai,.gptmkt-msg-v4.admin{background:#fff!important;color:#0f172a!important;border-bottom-left-radius:6px!important;}.gptmkt-msg-v4.user{background:linear-gradient(135deg,#2563eb,#7c3aed)!important;color:#fff!important;margin-left:auto!important;border-bottom-right-radius:6px!important;}
...
(function(){
  'use strict';
  var deferredPrompt=window.__gptMktDeferredPrompt||window.__mktDeferredPrompt||null, pollTimer=null, lastId=Number(localStorage.getItem('gptmkt_support_last_id')||0);
  function id(x){return document.getElementById(x)} function esc(s){return String(s||'').replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]})}
  function deviceId(){var v='';try{v=localStorage.getItem('mkt_device_id')||''}catch(e){} if(!v||v==='Đang tạo...'||v.length<8){v='MKT-'+Math.random().toString(36).slice(2,8).toUpperCase()+Date.now().toString().slice(-6);try{localStorage.setItem('mkt_device_id',v)}catch(e){}} try{document.cookie='mkt_device_id='+encodeURIComponent(v)+'; path=/; max-age='+(60*60*24*365*5)+'; SameSite=Lax'}catch(e){} return v.toUpperCase();}
  window.mktGetDeviceId=window.ensureDeviceId=deviceId;
  function sidebar(){return document.querySelector('.sidebar,.v2-sidebar,aside,[class*="sidebar"],[class*="side-bar"]')}
  function addDeviceCard(){var sb=sidebar(); if(!sb||id('gptMktDeviceCardV4'))return; var card=document.createElement('div');card.id='gptMktDeviceCardV4';card.innerHTML='<div class="t">ID THIẾT BỊ</div><div class="id">'+esc(deviceId())+'</div><div class="st">Đang hoạt động</div>'; var title=sb.querySelector('h1,.logo,.brand,.app-title')||sb.firstElementChild; if(title&&title.parentNode===sb&&title.nextSibling)sb.insertBefore(card,title.nextSibling);else sb.insertBefore(card,sb.firstChild);}
  function build(){
    if(!id('gptMktInstallDockV4')){var d=document.createElement('div');d.id='gptMktInstallDockV4';d.innerHTML='<button type="button" id="gptMktInstallBtnV4"><span class="ico">📱</span><span><b>GPT MKT</b><small>Tải xuống</small></span></button>';document.body.appendChild(d)}
    if(!id('gptMktChatBtnV4')){var c=document.createElement('button');c.type='button';c.id='gptMktChatBtnV4';c.innerHTML='🤖';document.body.appendChild(c)}
    if(!id('gptMktChatPanelV4')){var p=document.createElement('div');p.id='gptMktChatPanelV4';p.innerHTML='<div class="gptmkt-chat-head-v4"><div><b>GPT MKT Support</b><small>🟢 AI đang trực tuyến • '+esc(deviceId())+'</small></div><button type="button" class="gptmkt-chat-close-v4">×</button></div><div id="gptMktChatBodyV4"></div><div class="gptmkt-chat-input-v4"><input id="gptMktChatInputV4" placeholder="Nhập tin nhắn hỗ trợ..."><button type="button" id="gptMktSendV4">Gửi</button></div>';document.body.appendChild(p);append('ai','Xin chào 👋\nEm là GPT MKT Support. Anh/chị cần hỗ trợ tải app, kích hoạt Premium, thanh toán hay lỗi kỹ thuật ạ?')}
    if(!id('gptMktInstallSheetV4')){var s=document.createElement('div');s.id='gptMktInstallSheetV4';s.innerHTML='<div class="gptmkt-install-card-v4"><h3>📱 Cài GPT MKT vào điện thoại</h3><p id="gptMktInstallTextV4"></p><div class="actions"><button type="button" class="go" id="gptMktInstallGoV4">Cài ngay</button><button type="button" class="close" id="gptMktInstallCloseV4">Đóng</button></div></div>';document.body.appendChild(s)}
    addDeviceCard();
  }
  function append(role,msg){build();var b=id('gptMktChatBodyV4');if(!b)return;var m=document.createElement('div');m.className='gptmkt-msg-v4 '+role;m.innerHTML=esc(msg);b.appendChild(m);b.scrollTop=b.scrollHeight;}
  function openChat(){build();id('gptMktChatPanelV4').classList.add('open');startPoll()} function closeChat(){var p=id('gptMktChatPanelV4');if(p)p.classList.remove('open')} function toggleChat(){build();var p=id('gptMktChatPanelV4');p.classList.contains('open')?closeChat():openChat()}
  window.openFloatingBot=openChat;window.toggleFloatingBot=toggleChat;window.closeFloatingBot=closeChat;
  async function send(){build();var inp=id('gptMktChatInputV4');var text=(inp&&inp.value||'').trim();if(!text)return;if(inp)inp.value='';openChat();append('user',text);try{var r=await fetch('/support_send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({device_id:deviceId(),sender:'user',message:text})});var data=await r.json().catch(function(){return{}});if(data.id){lastId=Math.max(lastId,Number(data.id)||0);localStorage.setItem('gptmkt_support_last_id',String(lastId));}}catch(e){append('ai','Chưa gửi được về Web Admin. Vui lòng kiểm tra mạng hoặc chạy đúng file mới.')}setTimeout(function(){append('ai',reply(text))},280)}
  function reply(t){t=String(t||'').toLowerCase();if(t.indexOf('tải')>=0||t.indexOf('cài')>=0)return 'Anh/chị bấm nút GPT MKT bên trái. Android Chrome có thể cài trực tiếp; iPhone Safari chọn Chia sẻ → Thêm vào màn hình chính.';if(t.indexOf('premium')>=0||t.indexOf('thanh toán')>=0||t.indexOf('qr')>=0)return 'Anh/chị gửi ID thiết bị '+deviceId()+' kèm ảnh thanh toán để admin kích hoạt nhanh.';return 'Em đã nhận tin nhắn. Admin sẽ phản hồi trực tiếp trong khung chat này ạ.'}
  async function poll(){try{var r=await fetch('/support_poll?device_id='+encodeURIComponent(deviceId())+'&after_id='+encodeURIComponent(lastId),{cache:'no-store'});var d=await r.json().catch(function(){return{messages:[]}});(d.messages||[]).forEach(function(m){var mid=Number(m.id)||0;if(mid<=lastId)return;lastId=mid;localStorage.setItem('gptmkt_support_last_id',String(lastId));if(String(m.sender||'').toLowerCase()==='admin'){append('admin',m.message);openChat();}})}catch(e){}}
  function startPoll(){if(pollTimer)return;poll();pollTimer=setInterval(poll,3500)}
  function ios(){return /iphone|ipad|ipod/i.test(navigator.userAgent)} function android(){return /android/i.test(navigator.userAgent)} function standalone(){return matchMedia('(display-mode: standalone)').matches||navigator.standalone===true}
  function showGuide(){build();var txt=id('gptMktInstallTextV4'),go=id('gptMktInstallGoV4');if(ios()){txt.innerHTML='iPhone: mở bằng <b>Safari</b> → bấm nút <b>Chia sẻ</b> → chọn <b>Thêm vào màn hình chính</b> → <b>Thêm</b>.';go.textContent='Đã hiểu'}else if(deferredPrompt&&android()){txt.innerHTML='Bấm <b>Cài ngay</b>, sau đó chọn <b>Cài đặt</b> để đưa GPT MKT ra màn hình chính.';go.textContent='Cài ngay'}else{txt.innerHTML='Android Chrome: bấm menu <b>⋮</b> → <b>Cài đặt ứng dụng</b> hoặc <b>Thêm vào màn hình chính</b>.<br>iPhone Safari: <b>Chia sẻ</b> → <b>Thêm vào màn hình chính</b>.';go.textContent='Đã hiểu'}id('gptMktInstallSheetV4').classList.add('open')}
  async function install(){if(standalone())return;if(deferredPrompt&&android()){var dp=deferredPrompt;deferredPrompt=null;try{dp.prompt();await dp.userChoice}catch(e){}return}showGuide()}
  window.addEventListener('beforeinstallprompt',function(e){e.preventDefault();deferredPrompt=e;window.__gptMktDeferredPrompt=e;window.__mktDeferredPrompt=e;},{capture:true});
  document.addEventListener('click',function(e){var t=e.target;if(t.closest&&t.closest('#gptMktInstallBtnV4')){e.preventDefault();e.stopPropagation();install()} if(t.closest&&t.closest('#gptMktChatBtnV4,.bot-bubble,[data-open-support-bot]')){e.preventDefault();e.stopPropagation();toggleChat()} if(t.closest&&t.closest('#gptMktSendV4')){e.preventDefault();send()} if(t.closest&&t.closest('.gptmkt-chat-close-v4')){e.preventDefault();closeChat()} if(t.closest&&t.closest('#gptMktInstallCloseV4')){e.preventDefault();id('gptMktInstallSheetV4').classList.remove('open')} if(t.closest&&t.closest('#gptMktInstallGoV4')){e.preventDefault(); if(deferredPrompt&&android()){var dp=deferredPrompt;deferredPrompt=null;try{dp.prompt();dp.userChoice}catch(er){}}else{id('gptMktInstallSheetV4').classList.remove('open')}}},true);
  document.addEventListener('keydown',function(e){if(e.key==='Enter'&&e.target&&e.target.id==='gptMktChatInputV4'){e.preventDefault();send()}},true);
  function boot(){build();deviceId();startPoll();if('serviceWorker' in navigator){navigator.serviceWorker.register('/sw.js',{scope:'/'}).catch(function(){navigator.serviceWorker.register('/service-worker.js',{scope:'/'}).catch(function(){})})}}
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot);else boot();setTimeout(boot,600);setTimeout(boot,1800);setTimeout(addDeviceCard,3000);
})();
</script>
...
<!-- FINAL PATCH V130: Device ID card + Reliable support chat + Simple mobile PWA download -->
<style id="mkt-v130-device-chat-install">
  /* ID thiết bị dưới thương hiệu */
  .mkt-device-card-v130{
  ...
  .mkt-device-card-v130 .mkt-device-label{
  ...
  .mkt-device-card-v130 .mkt-device-value{
  ...
  .mkt-device-card-v130 .mkt-device-status{
  ...
  /* Ẩn các nút cài app cũ để tránh rối giao diện */
  .app-install-banner,#installAppBtn,.mkt-install-old,.mkt-install-sheet,
  #mktInstallSheet,.install-app-btn,.mkt-mobile-actions-old{
    display:none!important;
  }

  /* Nút tải xuống đơn giản, chỉ hiện trên điện thoại */
  .mkt-mobile-download-v130{
    ...
    box-shadow:0 12px 34px rgba(37,99,235,.35)!important;
    cursor:pointer!important;
    animation:mktDownloadFloatV130 2.2s ease-in-out infinite!important;
    touch-action:manipulation!important;
  }
  .mkt-mobile-download-v130 small{
    font-size:11px!important;
    font-weight:900!important;
    opacity:.94!important;
    margin-left:2px!important;
  }
  .mkt-mobile-download-v130 .mkt-dot{
    width:10px!important;height:10px!important;border-radius:50%!important;
    background:#22c55e!important;box-shadow:0 0 0 6px rgba(34,197,94,.16),0 0 12px rgba(34,197,94,.7)!important;
  }
  @keyframes mktDownloadFloatV130{0%,100%{transform:translateY(0)}50%{transform:translateY(-4px)}}

  .mkt-install-guide-v130{
    position:fixed!important;
    inset:0!important;
    z-index:2147483600!important;
    display:none!important;
  ...
  .mkt-install-guide-v130.open{display:flex!important;}
  .mkt-install-guide-box-v130{
    width:100%!important;
  ...
  .mkt-install-guide-box-v130 h3{
    margin:0 0 8px!important;
    font-size:20px!important;
    font-weight:1000!important;
    color:#0f172a!important;
  }
  .mkt-install-guide-box-v130 p{
    margin:0 0 12px!important;
    color:#475569!important;
    font-size:14px!important;
    line-height:1.45!important;
  }
  .mkt-install-steps-v130{
    background:#f8fafc!important;
  ...
  .mkt-install-guide-actions-v130{display:flex!important;gap:10px!important;}
  .mkt-install-guide-actions-v130 button{
    flex:1!important;
    border:0!important;
  ...
  .mkt-install-go-v130{background:linear-gradient(135deg,#2563eb,#7c3aed)!important;color:#fff!important;}
  .mkt-install-close-v130{background:#eef2ff!important;color:#312e81!important;}

  /* Bot luôn có thể click và mở panel */
  .floating-bot{
  ...
  .bot-panel{
    z-index:2147483403!important;
    pointer-events:auto!important;
  }
  .bot-panel.mkt-bot-open,
  body.mkt-support-open .bot-panel{
    display:block!important;
    opacity:1!important;
    visibility:visible!important;
  }
  .bot-input input,.bot-input button,.bot-actions button{
    pointer-events:auto!important;
  }

  @media(max-width:820px){
    .mkt-mobile-download-v130{display:inline-flex!important;}
    body{padding-top:54px!important;}
    .floating-bot{
      right:14px!important;
      bottom:calc(18px + env(safe-area-inset-bottom,0px))!important;
    }
    .bot-bubble{width:62px!important;height:62px!important;font-size:28px!important;}
    .bot-panel{
      position:fixed!important;
      left:10px!important;
    ...
    .bot-body{max-height:40vh!important;}
  }
</style>
...
<div class="mkt-install-guide-v130" id="mktInstallGuideV130" aria-hidden="true">
  <div class="mkt-install-guide-box-v130">
    ...
    <div class="mkt-install-guide-actions-v130">
      <button type="button" class="mkt-install-go-v130" id="mktInstallGoV130">Cài ngay</button>
      <button type="button" class="mkt-install-close-v130" id="mktInstallCloseV130">Đóng</button>
    </div>
  </div>
</div>

<script id="mkt-v130-device-chat-install-js">
(function(){
  'use strict';

  var deferredPrompt=null;
  var supportPollStarted=false;
  var lastSupportId=Number(localStorage.getItem('mkt_support_last_id')||0);
  ...
  function makeDeviceId(){
    ...
    if(!id || id==='Đang tạo...' || id.length<8){
      id='MKT-'+Math.random().toString(36).slice(2,8).toUpperCase()+Date.now().toString().slice(-6);
  ...
  function ensureDeviceCard(){
    var id=makeDeviceId();
    var existing=byId('mktDeviceCardV130');
    var brand=q('.v2-brand') || q('.sidebar .brand') || q('.sidebar') || q('.v2-sidebar') || q('[class*="sidebar"]');
    if(!brand) return;
    if(!existing){
      var card=document.createElement('div');
      card.id='mktDeviceCardV130';
      card.className='mkt-device-card-v130';
      card.innerHTML='<div class="mkt-device-label">ID thiết bị</div><div class="mkt-device-value" id="mktDeviceValueV130"></div><div class="mkt-device-status">Trạng thái: Dùng thử miễn phí 3 ngày</div>';
      var after=q('.v2-brand-sub', brand) || q('.v2-brand-title', brand) || brand.firstElementChild;
      if(after && after.parentNode===brand){ after.insertAdjacentElement('afterend', card); }
      else { brand.insertBefore(card, brand.firstChild ? brand.firstChild.nextSibling : null); }
    }
    var val=byId('mktDeviceValueV130');
    if(val) val.textContent=id;
    qa('#sidebarDeviceId,.device-id-value,[data-device-id-text]').forEach(function(el){el.textContent=id;});
  }
  ...
  function appendBot(sender,msg,msgId){
    var b=bodyBox(); if(!b) return;
    if(msgId && b.querySelector('[data-support-id="'+msgId+'"]')) return;
    var name=sender==='admin'?'Admin hỗ trợ':(sender==='user'?'Bạn':'Bot hỗ trợ');
    var cls=sender==='admin'||sender==='ai'?'bot-msg ai':'bot-msg';
    var div=document.createElement('div');
    div.className=cls;
    if(msgId) div.setAttribute('data-support-id',String(msgId));
  ...
  async function supportSend(text){
    ...
    try{
      var res=await fetch('/support_send',{
        method:'POST',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({device_id:makeDeviceId(),sender:'user',message:text})
      });
      var data=await res.json().catch(function(){return {};});
      if(data && data.id){
        lastSupportId=Math.max(lastSupportId,Number(data.id)||0);
        try{localStorage.setItem('mkt_support_last_id',String(lastSupportId));}catch(e){}
      }
    }catch(e){}
  }
  ...
  async function pollSupport(){
    try{
      var res=await fetch('/support_poll?device_id='+encodeURIComponent(makeDeviceId())+'&after_id='+encodeURIComponent(lastSupportId),{cache:'no-store'});
      var data=await res.json().catch(function(){return {messages:[]};});
      (data.messages||[]).forEach(function(m){
        var mid=Number(m.id)||0;
        if(mid<=lastSupportId) return;
  ...
  function startSupportPoll(){
    if(supportPollStarted) return;
    supportPollStarted=true;
    pollSupport();
    setInterval(pollSupport,3500);
  }

  function openBot(){
    var p=panel(); if(!p) return;
    p.classList.add('mkt-bot-open');
    p.style.display='block';
    p.style.opacity='1';
    p.style.visibility='visible';
    document.body.classList.add('mkt-support-open');
    startSupportPoll();
    setTimeout(scrollBot,80);
  }
  function closeBot(){
    var p=panel(); if(!p) return;
    p.classList.remove('mkt-bot-open');
    p.style.display='none';
    document.body.classList.remove('mkt-support-open');
  }
  function toggleBot(){
    var p=panel(); if(!p) return;
    if(p.classList.contains('mkt-bot-open') || p.style.display==='block') closeBot(); else openBot();
  }

  window.openFloatingBot=openBot;
  window.closeFloatingBot=closeBot;
  window.toggleFloatingBot=toggleBot;
  ...
  window.sendBotInput=function(){
    ...
    appendBot('user',text);
    supportSend(text);
    setTimeout(function(){appendBot('ai','Đã nhận tin nhắn của anh/chị. Admin sẽ phản hồi trực tiếp tại khung chat này.');},350);
    startSupportPoll();
  };
  ...
  document.addEventListener('click',function(e){
    var bot=e.target.closest('.bot-bubble,.floating-bot,[data-open-support-bot]');
    if(!bot) return;
    if(e.target.closest('.bot-panel')) return;
    e.preventDefault();
    e.stopPropagation();
    if(e.stopImmediatePropagation) e.stopImmediatePropagation();
    toggleBot();
  ...
  document.addEventListener('keydown',function(e){
    if(e.key==='Enter' && e.target && (e.target.id==='botInputText' || e.target.closest('.bot-input'))){
      e.preventDefault();
      window.sendBotInput();
    }
  },true);
  ...
  function guideText(){
    var intro=byId('mktInstallIntroV130'), steps=byId('mktInstallStepsV130'), go=byId('mktInstallGoV130');
    if(isIOS()){
      if(intro) intro.textContent='iPhone không cho website tự cài 1 chạm. Làm theo 3 bước dưới đây để đưa app ra màn hình chính.';
      ...
      if(intro) intro.textContent='Trình duyệt chưa bật popup cài tự động. Vẫn có thể cài bằng menu trình duyệt.';
      if(steps) steps.innerHTML='Android Chrome: bấm menu <b>⋮</b> → <b>Thêm vào màn hình chính</b> hoặc <b>Cài đặt ứng dụng</b>.';
      if(go) go.textContent='Đã hiểu';
    }
  }
  ...
  async function installApp(){
    ...
    guideText();
    var sheet=byId('mktInstallGuideV130');
    if(sheet) sheet.classList.add('open');
  }

  document.addEventListener('click',function(e){
    var btn=e.target.closest('#mktDownloadAppV130,[data-mkt-install-app],[data-install-app]');
    if(!btn) return;
    e.preventDefault();
    e.stopPropagation();
    if(e.stopImmediatePropagation) e.stopImmediatePropagation();
  ...
  document.addEventListener('click',function(e){
    if(e.target.closest('#mktInstallCloseV130')){
      var sheet=byId('mktInstallGuideV130'); if(sheet) sheet.classList.remove('open');
    }
    if(e.target.closest('#mktInstallGoV130')){
      var sheet=byId('mktInstallGuideV130');
      if(deferredPrompt && !isIOS()){
        var dp=deferredPrompt; deferredPrompt=null; dp.prompt(); dp.userChoice.catch(function(){});
  ...
  function boot(){
    ...
    startSupportPoll();
    if(isStandalone()){
      var b=byId('mktDownloadAppV130'); if(b) b.style.display='none';
    }
    if('serviceWorker' in navigator){
  ...
  if(document.readyState==='loading') document.addEventListener('DOMContentLoaded',boot); else boot();
  setTimeout(boot,700);
  setTimeout(ensureDeviceCard,1600);
})();
</script>
